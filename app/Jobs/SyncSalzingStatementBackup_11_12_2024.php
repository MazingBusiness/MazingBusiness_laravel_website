...
class SyncSalzingStatement implements ShouldQueue
{
    ...
    public function handle()
    {
        ...
        $currentMonth = date('m');
        $currentYear = date('Y');
        // $userAddressData = Address::where('user_id', '23760')->groupBy('gstin')->orderBy('acc_code','ASC')->get();
        $userAddressData = Address::where('acc_code',"!=","")->groupBy('gstin')->orderBy('acc_code','ASC')->get();
        ...
        foreach($userAddressData as $key=>$value){
            ...
            if ($overdue_response->successful()) {
                ...
                if(!empty($getData) AND isset($getData['data']) AND !empty($getData['data'])){
                    ...
                    // echo "<pre>";print_r(json_encode($getData));
                    // echo '---'.$dueAmount;
                    // echo '---'.$closeDrOrCr;
                    // echo '---'.$overdueAmount;
                    // echo '---'.$overdueDrOrCr;
