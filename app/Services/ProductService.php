...
class ProductService {
  public function store(array $data) {
    ...
    $generic_name       = array();
    if ($collection['generic_name'][0] != null) {
      foreach (json_decode($collection['generic_name'][0]) as $key => $tag) {
        array_push($generic_name, $tag->value);
      }
    }
    $collection['generic_name'] = implode(',', $generic_name);
    ...
    if (count($combinations[0]) > 0) {
      foreach ($combinations as $key => $combination) {
        ...
        unset($collection['part_no_' . str_replace('.', '_', $str)]);
        unset($collection['model_no_' . str_replace('.', '_', $str)]);
        unset($collection['seller_sku_' . str_replace('.', '_', $str)]);
        unset($collection['price_' . str_replace('.', '_', $str)]);
        unset($collection['carton_price_' . str_replace('.', '_', $str)]);
        unset($collection['piece_per_carton_' . str_replace('.', '_', $str)]);
        unset($collection['qty_' . str_replace('.', '_', $str)]);
        unset($collection['seller_stock_' . str_replace('.', '_', $str)]);
        unset($collection['cbm_' . str_replace('.', '_', $str)]);
        unset($collection['carton_cbm' . str_replace('.', '_', $str)]);
        unset($collection['length_' . str_replace('.', '_', $str)]);
        unset($collection['breadth_' . str_replace('.', '_', $str)]);
        unset($collection['height_' . str_replace('.', '_', $str)]);
        unset($collection['weight_' . str_replace('.', '_', $str)]);
