...
Route::group(['prefix' => 'v2/auth', 'middleware' => ['app_language']], function () {
    ...
    Route::post('manager_clients', 'App\Http\Controllers\Api\V2\AuthController@manager_clients');
    ...
    Route::post('login_otp', 'App\Http\Controllers\Api\V2\AuthController@login_otp')->name('loginOTP'); // Excluded from CSRF protection

    Route::post('download-excel', 'App\Http\Controllers\Api\V2\AuthController@manager_clients');

    //Customer add by Magager
    Route::post('get_warehouse', 'App\Http\Controllers\Api\V2\AuthController@getWarehouse')->name('getWarehouse');
    Route::post('check_gst_number', 'App\Http\Controllers\Api\V2\AuthController@checkGstNumber')->name('checkGstNumber');
    Route::post('check_aadhar_card_number', 'App\Http\Controllers\Api\V2\AuthController@checkAadharCardNumber')->name('checkAadharCardNumber');
    Route::post('check_email', 'App\Http\Controllers\Api\V2\AuthController@checkEmail')->name('checkEmail');
    Route::post('check_phone', 'App\Http\Controllers\Api\V2\AuthController@checkPhone')->name('checkPhone');
    Route::post('check_pin_code', 'App\Http\Controllers\Api\V2\AuthController@checkPinCode')->name('checkPinCode');
    Route::post('create_customer', 'App\Http\Controllers\Api\V2\AuthController@createCustomer')->name('createCustomer');
    Route::post('registration_customer', 'App\Http\Controllers\Api\V2\AuthController@registrationCustomer')->name('registrationCustomer');

});

// use App\Http\Controllers\Api\V2\AuthController;

// Route::post('test-login-otp', [AuthController::class, 'login_otp']);
...
Route::group(['prefix' => 'v2', 'middleware' => ['app_language']], function () {
    ...
    // Route::post('api/saleszing/orders/status', 'App\Http\Controllers\Api\V2\OrderController@orderStatusSaleszing');

    // Route::post('user/shipping/update', 'App\Http\Controllers\Api\V2\AddressController@updateShippingAddress');
    Route::group(['middleware' => ['app_user_unbanned']], function () {
        ...
        Route::get('all-coupons', 'App\Http\Controllers\Api\V2\CheckoutController@allCoupons')->middleware('auth:sanctum');
    ...
    Route::post('payment/webhook', 'App\Http\Controllers\Api\V2\PaymentController@webhook')->name('webhook');
    Route::post('payment/generate-url', 'App\Http\Controllers\Api\V2\PaymentController@generateUrl')->name('generateUrl');
    ...
    Route::get('professions', 'App\Http\Controllers\Api\V2\CategoryController@professions');
    ...
    Route::get('saleszing/customers', 'App\Http\Controllers\Api\V2\UserController@customerSaleszing');
    ...
    Route::get('products/saleszing/seller/{id}', 'App\Http\Controllers\Api\V2\ProductController@sellerSaleszing');
    Route::get('products/importClients', 'App\Http\Controllers\Api\V2\ProductController@importClients');
    ...
    Route::get('products/new', 'App\Http\Controllers\Api\V2\ProductController@new');
    ...
    Route::post('products/looking-for-product', 'App\Http\Controllers\Api\V2\ProductController@lookingForProduct');
    // Quick order
    Route::get('products/quick-order/category-groups', 'App\Http\Controllers\Api\V2\QuickOrderController@quickOrderCategoryGroupList');
    Route::get('products/quick-order/categories/{category_group_id?}', 'App\Http\Controllers\Api\V2\QuickOrderController@quickOrderCategories');
    Route::get('products/quick-order/brands/{category_group_id?}', 'App\Http\Controllers\Api\V2\QuickOrderController@quickOrderBrands');
    Route::get('products/quick-order/brands', 'App\Http\Controllers\Api\V2\QuickOrderController@quickOrderBrands');
    Route::get('products/quick-order/{category_group_id?}', 'App\Http\Controllers\Api\V2\QuickOrderController@quickOrderProducts');
    ...
    Route::get('products/search', 'App\Http\Controllers\Api\V2\ProductController@search')->name('api.products.search');
    Route::post('products/variant/price', 'App\Http\Controllers\Api\V2\ProductController@variantPrice');
    ...
    Route::get('products/{id}', 'App\Http\Controllers\Api\V2\ProductController@show')->middleware('auth:sanctum');

    Route::post('download-excel', 'App\Http\Controllers\Api\V2\ProductController@downloadExcel')->name('downloadExcel');
    Route::post('download-pdf', 'App\Http\Controllers\Api\V2\ProductController@downloadPdf')->name('downloadPdf');
    Route::get('/pdf-status/{filename}', function ($filename) {
        $report = App\Models\PdfReport::where('filename', $filename)->first();    
        if (!$report) {
            return response()->json(['message' => 'PDF report not found'], 404);
        }    
    ...
    Route::get('whatsapp-msg', 'App\Http\Controllers\Api\V2\ProductController@whatsappMessage')->name('whatsappMessage');
    Route::post('whatsapp-send-msg', 'App\Http\Controllers\Api\V2\ProductController@whatsappSendMsgStatus')->name('whatsappSendMsgStatus');
    ...
    Route::get('sliders', 'App\Http\Controllers\Api\V2\SliderController@getSliders');
    ...
    //Saleszing Part
    Route::post('client-push', 'App\Http\Controllers\Api\V2\SaleszingController@clientPush')->name('clientPush');
    Route::post('order-push', 'App\Http\Controllers\Api\V2\SaleszingController@orderPush')->name('orderPush');
    Route::post('item-push', 'App\Http\Controllers\Api\V2\SaleszingController@itemPush')->name('itemPush');

    //ChatBot API
    Route::post('user-status', 'App\Http\Controllers\Api\V2\ChatBotController@userStatus')->name('userStatus');
    Route::post('user-credential', 'App\Http\Controllers\Api\V2\ChatBotController@userCredential')->name('userCredential');
    Route::post('user-aadhar-card', 'App\Http\Controllers\Api\V2\ChatBotController@userAadhaarCard')->name('userAadhaarCard');
    Route::post('user-registration-using-aadhar-card', 'App\Http\Controllers\Api\V2\ChatBotController@userRegistrationUsingAadhaarCard')->name('userRegistrationUsingAadhaarCard');
    Route::post('get-head-manager-details-using-pincode', 'App\Http\Controllers\Api\V2\ChatBotController@getHeadManagerDetailsUsingPincode')->name('getHeadManagerDetailsUsingPincode');
    Route::post('get-under-head-manager-details-using-phone', 'App\Http\Controllers\Api\V2\ChatBotController@getUnderHeadManagerDetailsUsingPhone')->name('getUnderHeadManagerDetailsUsingPhone');
    Route::post('user-detaile-using-phone-number', 'App\Http\Controllers\Api\V2\ChatBotController@userDetailsUsingphoneNumber')->name('userDetailsUsingphoneNumber');
    Route::post('gst-validation', 'App\Http\Controllers\Api\V2\ChatBotController@gatValidation')->name('gatValidation');
    Route::post('user-registration-using-gst', 'App\Http\Controllers\Api\V2\ChatBotController@userRegistrationUsingGST')->name('userRegistrationUsingGST');
    Route::post('manager-approval', 'App\Http\Controllers\Api\V2\ChatBotController@managerApproval')->name('managerApproval');
    Route::get('update-unapproved-status/{user_phone_number}', 'App\Http\Controllers\Api\V2\ChatBotController@updateUnapprovedStatus')->name('updateUnapprovedStatus');
    Route::post('user-registration-by-manager-using-gst', 'App\Http\Controllers\Api\V2\ChatBotController@userRegistrationByManagerUsingGST')->name('userRegistrationByManagerUsingGST');
    Route::post('user-registration-by-manager-using-aadhar-card', 'App\Http\Controllers\Api\V2\ChatBotController@userRegistrationByManagerUsingAadharCard')->name('userRegistrationByManagerUsingAadharCard');
});
