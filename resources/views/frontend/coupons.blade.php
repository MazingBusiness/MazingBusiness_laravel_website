                        ...
                        <div class="row gutters-5">
                          ...
                          @foreach ($products as $key => $product)
                            @if ($product != null && $key < 3)
                              <a href="{{ route('product', $product->slug) }}" class='col-4 text-center text-reset mb-3'
                                target="_blank">
                                <img
                    ...
                    <div class="col-md-8 text-dark align-self-center">
                      <div class="px-4 py-3">
                        ...
                        <span class="h5 text-center d-block m-auto bg-soft-info p-3 rounded">
                          @if ($coupon->discount_type == 'amount')
                            {{ translate('Min Spend ') }} {{ single_price($order_discount->min_buy) }}
                            {{ translate('from') }} {{ $name }} {{ translate('to get') }}
                            {{ single_price($coupon->discount) }} {{ translate('OFF on total orders') }}
                          @else
                            {{ translate('Min Spend ') }} {{ single_price($order_discount->min_buy) }}
                            {{ translate('from') }} {{ $name }} {{ translate('to get') }}
                            {{ $coupon->discount }}% {{ translate('OFF on total orders') }}
                          @endif
                        </span>
                      </div>
                    </div>
