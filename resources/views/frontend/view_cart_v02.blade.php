                      ...
                      <ul class="list-group list-group-flush">
                        ...
                        @foreach ($carts as $key => $cartItem)                    
                          ...
                          <li class="list-group-item px-0 px-lg-3" id="cartRow_{{ $cartItem['id'] }}">
                            <span class="opacity-70" style="color:#f00;" id="increasePriceText_{{ $cartItem['id'] }}"></span>
...
@section('script')
  <script type="text/javascript">
    ...
    //       $('#increasePriceText_' + cart_id).html(data.increasePriceText || '');
    //       alert(data.increasePriceText);
    ...
    function updateQuantity(cart_id, quantity) {
      ...
      $.ajax({
        url: '{{ route('cart.updateQuantityV02') }}',
        method: 'POST',
        data: {
          _token: '{{ csrf_token() }}',
          id: cart_id,
      ...
      .done(function (data) {
        // 1) Replace big chunks FIRST (these may contain the target span)
        updateNavCart(data.nav_cart_view, data.cart_count);
        $('#cart_split_bill').html(data.cart_summary);
        $('#cart-summary').html(data.html);

        // 2) Now update row-specific bits in the NEW DOM
        const cid = (window.CSS && CSS.escape) ? CSS.escape(String(cart_id)) : cart_id;

        // Scope to the row to avoid touching “save for later” etc.
        const $row = $('#cartRow_' + cid);

        $row.find('#item_sub_total_span_' + cid).html(data.item_sub_total);
        $('#span_sub_total').html(data.span_sub_total);

        // This was not showing because the span got replaced above
        $row.find('#increasePriceText_' + cid).html(data.increasePriceText || '');

        if ($('#spanPrice_' + cid).length) {
          $('#spanPrice_' + cid).html(data.price);
        }
        if ($('#updatePrice_' + cid).length) {
          $('#updatePrice_' + cid).html(data.update_price);
        }

        // Re-init widgets after DOM changes
        initAizCarousel();
      })
      .fail(function (xhr) {
        console.error('updateQuantity error:', xhr?.responseText || xhr);
      })
      .always(function () {
        $('.ajax-loader').css("visibility", "hidden");
      });
