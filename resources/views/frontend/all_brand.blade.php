        ...
        <div class="row row-cols-xxl-6 row-cols-xl-5 row-cols-lg-4 row-cols-md-3 row-cols-2 gutters-10">
          @foreach ($brands as $brand)
			@php
				...
				// Fetch brand logo file_name
				$brand_logo_file = \App\Models\Upload::where('id', $brand->logo)->value('file_name');
				$brand_logo_path = $brand_logo_file
							? $baseUrl . '/' . $brand_logo_file
							: url('public/assets/img/placeholder.jpg');
			@endphp
            <div class="col text-center">
              <a href="{{ route('products.brand', $brand->slug) }}"
                class="d-block border-light hov-shadow-md mb-3 rounded border p-3">
                <img src="{{ $brand_logo_path  }}" class="lazyload h-70px mw-100 mx-auto"
                  alt="{{ $brand->getTranslation('name') }}">

              </a>
              <div class="brand-name">{{ $brand->getTranslation('name') }}</div>
            </div>
