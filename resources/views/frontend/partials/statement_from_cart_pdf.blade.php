    ...
    <tr>
        <td width="50%" style="text-align: center; font-family: Arial, sans-serif; font-size: 14px; color: #174e84;">
            ...
            <span style="font-size: 12px; color: {{ $overdueAmount > 0 ? '#ff0707' : '#333' }};">
                ₹{{ number_format($overdueAmount, 2) }} {{ $overdueDrOrCr }} <!-- Overdue Balance -->
            </span>
        </td>
        <td width="50%" style="text-align: center; font-family: Arial, sans-serif; font-size: 14px; color: #174e84;">
            <strong>Due Amount</strong><br>
            <span style="font-size: 12px; color: #333;">₹{{ number_format($dueAmount, 2) }} {{ $dueDrOrCr }}</span> <!-- Replacing 'Overdue From' with Due Amount -->
        </td>
    </tr>
</table>
<!-- Date Range Information -->
<p style="font-size: 14px; text-align: center; margin-top: 20px;">
    <strong>{{ date('d-M-y', strtotime($from_date)) }} to {{ date('d-M-y', strtotime($to_date)) }}</strong>
</p>

<!-- Transaction Table -->
@if (count($statementArray) > 0)
    @foreach($statementArray as $key => $statementData)
        @if(isset($statementData) AND count($statementData) > 0 )
            <p style="font-size: 14px; text-align: center; margin-top: 20px;">
                <strong>Company Name : </strong> {{$statementData['cmp_name']}}
            </p>
            <table width="100%" border="1" cellspacing="0" cellpadding="5" style="border-collapse: collapse; margin-top: 25px; font-family: Arial, sans-serif; border: 1px solid #ccc;">
                ...
                <tbody>
                    @php
                        ...
                        $closingCrBalance = 0;
                    @endphp
                    @foreach($statementData['statement'] as $transaction)
