<div class="container">
  <div class="row">
    <div class="col-lg-8">
      <h2 class="shadow-sm bg-white p-4 rounded">
        Saved For Later ({{ count($cartSaveForLater) }})
        <span style="width:auto;" class="badge badge-info ml-2">Manager-41</span>
      </h2>
      ...
      @if(count($cartSaveForLater) > 0)
        <div class="row align-items-center">
          ...
          <div class="col-md-8 text-center text-md-right">
            ...
            <button class="btn btn-success fw-600" onclick="moveAllCheckedItemToCart(event)">
              Move All Checked Item To Cart
            </button>

            @if($cash_and_carry_item_flag_for_later == 1)
              <button class="btn btn-info fw-600" onclick="moveAllNoCreditItemToCart(event)">
        ...
        <section class="mb-4">
          <div class="px-2 py-4 px-md-4 py-md-3 bg-white shadow-sm rounded">
            ...
            <div class="aiz-carousel gutters-10 half-outside-arrow"
                 data-items="5" data-xl-items="5" data-lg-items="6"
              ...
              @if(count($cartSaveForLaterCategory) > 0)
                @foreach($cartSaveForLaterCategory as $catKey=>$catValue)
                  <span class="shadow-sm p-2 bg-white rounded"
                        style="cursor:pointer;"
                        onclick="sortByCategoryIdInSaveForLater(event, {{ $catValue['category_id'] }})">
                    {{ $catValue['category_name'] }} ({{ $catValue['product_count'] }})
                  </span>
                @endforeach
              @endif
            </div>
          ...
          <div class="shadow-sm bg-white p-3 p-lg-4 rounded text-left">
            <div class="mb-4">
              <div class="row gutters-5 d-none d-lg-flex border-bottom mb-3 pb-3">
                <div class="col-md-1 fw-600 text-center">
                  <input type="checkbox" id="select-all-from-save-for-later"> {{-- unique id --}}
                </div>
              ...
              <ul class="list-group list-group-flush">
                ...
                @foreach ($cartSaveForLater as $key => $cartItem)
                  @php
                      ...
                      $product_stock = $product ? $product->stocks->where('variant', $cartItem['variation'])->first() : null;
                      $ppc = $product_stock ? $product_stock->piece_per_carton : 1;
                  @endphp
                    ...
                    <div class="row gutters-5">
                      <div class="col-lg-1 d-flex">
                        <span class="mr-2 ml-0">
                          <input type="checkbox"
                                 ...
                                 name="{{ $cartItem['id'] }}"
                                 value="{{ $cartItem['id'] }}"
                                 class="form-control move-to-cart-checkbox"> {{-- unique class --}}
                        </span>
                      </div>
                      ...
                      <div class="col-lg-3 d-flex">
                        <span class="mr-2 ml-0">
                          ...
                          @endphp
                          <img src="{{ $product_thumbnail_path }}" class="img-fit size-60px rounded"
                      ...
                      <div class="col-lg-2 col-6 order-5 order-lg-0  text-center">
                        ...
                        <a href="javascript:void(0)"
                           onclick="removeFromSaveForLaterView(event, {{ $cartItem['id'] }})"
                           class="btn btn-icon btn-sm btn-soft-primary btn-circle">
                          <i class="las la-trash"></i>
                        </a>
                        <a href="javascript:void(0)"
                           onclick="moveToCart(event, {{ $cartItem['id'] }})"
                           class="btn btn-icon btn-sm btn-soft-primary btn-circle"
                           title="Move to cart.">
