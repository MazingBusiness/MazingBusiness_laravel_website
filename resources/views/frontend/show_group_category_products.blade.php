...
<style>
  ...
  <section class="mb-4 pt-3">
                            ...
                            @foreach ($categoryGroup as $category)
                              ...
                              @endphp
                                ...
                                <li style="" class="mb-2 ml-1 fs-13 fw-600">
                                    <a style="color:#666666; text-decoration: none;" href="{{ route('group.category.products', ['category_group__id' => $category->id]) }}">
                                    ...
                                    <ul id="category_{{ $category->id }}" class="collapse sub-categories ml-3 {{ $isExpanded ? 'show' : '' }}"> <!-- Collapse sub-categories -->
                                        @php
    ...
    <a class="text-reset fs-12 fw-400" href="{{ route('products.category', $subcategory->slug) }}">
        {{ $subcategory->name }}
    </a>
@else
    <span class="text-muted fs-12 fw-400">{{ $subcategory->name }}</span>
@endif
          ...
          <div class="col-xl-8">
           ...
           <ul class="breadcrumb bg-transparent p-0">
              ...
              @if (!isset($category_group_id))
                  <li class="breadcrumb-item fw-600 text-dark">
                      <a class="text-reset" href="{{ route('search') }}">{{ translate('All Categories') }}</a>
                  </li>
              @else
                  <li class="breadcrumb-item opacity-50">
                      <a class="text-reset" href="{{ route('search') }}">{{ translate('All Groups') }}</a>
                  </li>
                  <li class="breadcrumb-item opacity-50">
                      @php
                          // Fetch the category group name
                          $categoryGroup = \App\Models\CategoryGroup::find($category_group_id);
                      @endphp
                      @if ($categoryGroup)
                          <a class="text-reset" href="{{ route('group.category.products', ['category_group__id' => $category_group_id]) }}">
                              {{ $categoryGroup->name }}
                          </a>
                      @endif
                  </li>
              ...
              @if (isset($category_group_id))
                  @php
                      // Fetch the categories within the group
                      $category = \App\Models\Category::where('category_group_id', $category_group_id)->first();
                  @endphp
                  @if ($category)
