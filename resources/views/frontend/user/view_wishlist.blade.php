          ...
          <div class="card mb-2 shadow-sm">
            <div class="card-body">
              ...
              @php
                ...
                // Fetch file_name for the product thumbnail image (assuming $wishlist->product->thumbnail_img contains the ID of the upload)
                $wishlist_thumbnail = \App\Models\Upload::where('id', $wishlist->product->thumbnail_img)->value('file_name');
                $wishlist_thumbnail_path = $wishlist_thumbnail
                            ? $uploads_base_url . '/' . $wishlist_thumbnail
                            : url('public/assets/img/placeholder.jpg');
              @endphp
              <img src="{{ $wishlist_thumbnail_path }}" class="img-fit h-140px h-md-200px">
              ...
              <div class=" fs-14">
                @if (Auth::user()->warehouse_id)
                  @if (home_base_price($wishlist->product) != home_discounted_base_price($wishlist->product))
                    <del class="fw-600 opacity-50 mr-1">{{ home_base_price($wishlist->product) }}</del>
                  @endif
                  <span class="fw-700 text-primary">{{ home_discounted_base_price($wishlist->product) }}</span>
                @else
                  <small><a href="{{ route('login') }}">Complete profile to check prices.</a></small>
                @endif
              </div>
            </div>
