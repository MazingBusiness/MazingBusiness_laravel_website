                        ...
                        @foreach ($orders as $key => $order)
                            @if (count($order->orderDetails) > 0)
                                <tr>
                                    ...
                                    <td>
                                        @if($order->bill_amount != "")
                                            {{ single_price($order->bill_amount) }}
                                        @else
                                            {{ format_price_in_rs($order->grand_total) }}
                                        @endif
                                    </td>
                                    <td>
                                    ...
                                    <td class="text-right">
                                        @if ($order->delivery_status == 'pending' && $order->payment_status == 'unpaid' AND $order->approved == false)
                                            <a href="javascript:void(0)" class="btn btn-soft-danger btn-icon btn-circle btn-sm confirm-delete" data-href="{{route('purchase_history.destroy', $order->id)}}" title="{{ translate('Cancel Order') }}">
