      ...
      <div class="bg-grad-2 text-white rounded-lg mb-4 overflow-hidden">
        ...
        <div class="px-3 pt-3">
            @php
                ...
                $totalOrders = $orders->count(); // Total number of orders

                $totalProducts = 0;
                foreach ($orders as $order) {
                    $totalProducts += $order->orderDetails()->count(); // Count products in each order
                }
            @endphp
            <div class="h3 fw-700">{{ $totalOrders }} {{ __('Order(s)') }}</div>
            <div class="opacity-50">{{ __('Total orders placed') }}</div>

            {{-- <div class="h3 fw-700 mt-3">{{ $totalProducts }} {{ __('Product(s)') }}</div>
            <div class="opacity-50">{{ __('Total products in your orders') }}</div> --}}
        </div>
    ...
    <div class="col-md-4">
        <div class="bg-grad-3 text-white rounded-lg mb-4 overflow-hidden">
          ...
          <div class="px-3 pt-3">
              ...
              @endphp
              <div class="h3 fw-700">{{ $totalOrders }} {{ __('Pending(s)') }}</div>
              <div class="opacity-50">{{ __('Total Pending Order') }}</div>
  ...
  {{-- <div class="row gutters-10">
    <div class="col-md-12">
      <div class="card">
        <div class="card-header">
          <h6 class="mb-0">{{ translate('Default Shipping Company') }}</h6>
        </div>
        <div class="card-body">
          @if (Auth::user()->addresses != null)
            ...
            @if ($address != null)
              <ul class="list-unstyled mb-0">
                <li class="pb-2"><span>{{ translate('Company Name') }} : {{ $address->company_name }}</span></li>
                @if ($address->gstin)
                  <li class="pb-2"><span>{{ translate('GSTIN') }} : {{ $address->gstin }}</span></li>
                @endif
                <li><span>{{ translate('Address') }} : {{ $address->address }}, {{ $address->city->name }} -
                    {{ $address->postal_code }}, {{ $address->state->name }}, {{ $address->country->name }}</span></li>
              </ul>
            @endif
          @else
            No Company set to Default
          @endif
        </div>
      </div>
    </div>
        ...
        <div class="card">
          ...
          <div class="card-body text-center">
            ...
            @if ($customer_package != null)
              ...
              <p class="mb-1 text-muted">{{ translate('Product Upload') }}: {{ $customer_package->product_upload }}
                {{ translate('Times') }}</p>
    ...
    <div class="row gutters-10">
        <div class="col-md-12">
          <div class="card">
            <div class="card-header">
              <h6 class="mb-0">{{ translate('Graph Representation') }}</h6>
            </div>
            <div class="card-body">
