  ...
  <div class="card">
    ...
    <div class="card-body">
      ...
      <div class="col-md-3 d-flex  align-items-center">
            ...
            <!-- Download PDF Button -->
            <a style="position:relative;right: 21px;" href="{{ route('impexOrderPdf', ['order_code' => $order->order_code]) }}" class="btn btn-primary mb-3 ">
                Download PDF
            </a>
        </div>
          ...
          <table>
            <tbody>
              ...
              @if ($order->advance_amount != NULL)
                <tr>
                  <td class="text-main text-bold">
                    <strong>{{ translate('Advance Amount') }}</strong>
                  </td>
                  <td class="text-right">
                    <strong><span id='mainGrandTotal'>{{ $order->currency.' '.$order->advance_amount }}</span></strong>
                  </td>
                </tr>
              @endif
              {{-- <tr>
          ...
          <table class="table-bordered aiz-table invoice-summary table">
            <thead>
              <tr class="bg-trans-dark">
                ...
                <th data-breakpoints="lg" class="min-col text-uppercase text-center">
                  {{ translate('Final Unit Price') }}
                </th>
                <th data-breakpoints="lg" class="min-col text-uppercase text-center" width="10%">
                  {{ translate('Total') }}
                </th>
              </tr>
            </thead>
                ...
                <tr id="tr_{{$orderDetail->id}}">
                  ...
                  <td>
                    @if ($orderDetail->name != null)
                      ...
                      <br>
                      <small id="small_days_of_delivery_{{$orderDetail->id}}">
                        <strong>Purchase Time Brand : </strong>{{ $orderDetail->purchase_time_brand }}
                          @if($orderDetail->purchase_time_brand_name != "")
                            {{' : '.$orderDetail->purchase_time_brand_name}}
                          @endif
                      </small>
                      <br>
                      <small id="small_brand_{{$orderDetail->id}}">
                        <strong>Final Brand : </strong>
                      ...
                      <br>
                      <small>
                      <strong>{{ translate('Part No.') }}:</strong> {{ $orderDetail->product->part_no }}
                      </small>
                      @if($orderDetail->days_of_delivery != NULL)
                        <br/><small id="small_days_of_delivery_{{$orderDetail->id}}">
                          <strong>Days of Delivery : </strong>{{ $orderDetail->days_of_delivery }}
                        </small>
                      @endif
                      @if($orderDetail->comment != NULL)
                        <br/><small id="smallcomment_{{$orderDetail->id}}">
