...
<div class="card">
    <form class="" action="" id="sort_orders" method="GET">
        ...
        <div class="card-header row gutters-5">
            ...
            <div class="col-lg-3 dropdown mb-2 mb-md-0">
                <button class="btn border dropdown-toggle" type="button" data-toggle="dropdown">
                    {{translate('Bulk Action')}}
                </button>
                <div class="dropdown-menu dropdown-menu-right">
                    <a class="dropdown-item" href="#" onclick="bulk_delete()"> {{translate('Delete selection')}}</a>
                    <a class="dropdown-item" href="#" onclick="bulk_push()"> {{translate('Selected Order Push into Salzing')}}</a>
                </div>
            </div>
            ...
            <table class="table aiz-table mb-0">
                <thead>
                    <tr>
                        ...
                        <?php /* <th class="text-right" width="1%">{{translate('Send')}}</th>  */ ?>
                        <th class="text-right" width="1%">{{translate('Salzing Order')}}</th> <!-- New column header -->
                    </tr>
                </thead>
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                            ...
                            {{ $order->user->user_warehouse->name }}
                        </td>
                        <td>
...
@section('script')
    <script type="text/javascript">
        ...
        function bulk_push() {
            // Collect all checked checkboxes with class 'check-one'
            var selectedIds = [];
            ...
            if (selectedIds.length > 0) {
                var conf = confirm('Are you sure you want to push all selected orders to Salezing?');
                if (conf == true) {
                    $.ajax({
                        headers: {
                            'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                        },
                        url: "{{ route('bulkOrderPushToSalezing') }}",
                        type: 'POST',
                        data: { selectedIds: selectedIds }, // Correctly pass data here
                        success: function (response) {
                            if (response == 1) {
                                location.reload();
                            }
