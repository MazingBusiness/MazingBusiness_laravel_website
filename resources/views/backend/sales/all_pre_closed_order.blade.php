...
<div class="card">
    <form class="" action="" id="sort_orders" method="GET">
        ...
        <div class="card-header row gutters-5">
            <div class="col">
                <h5 class="mb-md-0 h6">{{ translate('All Pre Closed Orders') }}</h5>
            </div>
                ...
                <tbody>
                    @foreach ($orderData as $key => $order)
                        <tr>
                            ...
                            <td>
                                {{ single_price($order->sub_order_details->sum('price')) }}
                            </td>
                            <td>
                            ...
                            <td class="text-right">
                                ...
                                <a class="btn btn-soft-primary btn-icon btn-circle btn-sm" href="{{ route('order.preClosedOrderDetails', $order->id) }}" title="{{ translate('View') }}">
                                    <i class="las la-eye"></i>
                                </a>
                                ...
                                <i class="las la-undo-alt undo_pre_close" style="font-size: 30px; color:#69d422; cursor:pointer;" title="Undo Pre Close" data-sub_order_id="{{ $order->id }}" data-order_id="{{ $order->order_id }}"></i>
                            </td>
                        </tr>
                    @endforeach
                </tbody>
...
<script>
    ...
    $(document).on("click", ".undo_pre_close", function () {
        ...
        if(conf == true){
            $.ajax({
                url: "{{ route('order.undoPreCloseSubOrder') }}", // Laravel route
                type: "POST",
                data: {
                    order_id: order_id,
