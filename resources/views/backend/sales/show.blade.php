...
@php
    $allowedUserIds = [1, 180, 169,25606,27604];
@endphp
            ...
            @if (auth()->user()->can('update_order_payment_status'))
              <select class="form-control aiz-selectpicker" data-minimum-results-for-search="Infinity"
                ...
                @unless ($payment_status == 'paid')
                  <option value="request-details" @if ($payment_status == 'request-details') selected @endif>
                    {{ translate('Request Payment Details') }}
                  </option>
                @endunless
              </select>
            @else
                  ...
                  <td>
                    @if ($orderDetail->product != null)
                      ...
                      <small>
                        ...
                        @endphp
                        {{ translate('Part No.') }}: {{ $product_stock['part_no'] }}
                      </small>
                    @else
                  ...
                  <td>
                    ...
                    @elseif($order->shipping_type == 'carrier')
                      @if ($order->carrier != null)
                        @if ($order->carrier->id == 372)
                          {{ $order->additional_info }} ({{ translate('Other Carrier') }})
                          <br>
      ...
      <div class="clearfix float-right">
        <table class="table">
          <tbody>
            ...
            <tr>
              <td>
                <strong class="text-muted">{{ translate('Payment Discount') }} :</strong>
              </td>
              <td>
                {{ single_price($order->payment_discount) }}
              </td>
            </tr>
            <tr>
              <td>
