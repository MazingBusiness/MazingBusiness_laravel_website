...
@section('content')
  <div class="aiz-titlebar text-left mt-2 mb-3">
    <div class="row align-items-center">
      ...
      @if (auth()->user()->can('add_new_seller'))
          ...
          <tbody>
            @foreach ($sellers as $key => $seller)
              <tr>
                ...
                <td>{{ $seller->user->warehouse->name }}</td>
                <td>{{ $seller->user->phone }}</td>
                <td>{{ $seller->user->email }}</td>
                <td>{{ $seller->gstin }}</td>
                <td>
                  <label class="aiz-switch aiz-switch-success mb-0">
                    <input @can('approve_seller') onchange="update_approved(this)" @endcan value="{{ $seller->id }}"
                      type="checkbox" <?php if ($seller->verification_status == 1) {
                          echo 'checked';
                      } ?>
                      @cannot('approve_seller') disabled @endcan
                        ...
                        <td>{{ $seller->warehouseproducts->count() }}</td>
                        <td>
                            @if ($seller->admin_to_pay >= 0)
                                {{ single_price($seller->admin_to_pay) }}
                            @else
                                {{ single_price(abs($seller->admin_to_pay)) }} ({{ translate('Due to Admin') }})
                            @endif
                        </td>
                        <td>
                            <div class="dropdown">
                                <button type="button" class="btn btn-sm btn-circle btn-soft-primary justify-content-center btn-icon dropdown-toggle no-arrow" data-toggle="dropdown" href="javascript:void(0);" role="button" aria-haspopup="false" aria-expanded="false">
                                ...
                                <div class="dropdown-menu dropdown-menu-right dropdown-menu-xs">
                                    ...
                                    @endcan
                                    @can('download_seller_products')
                                        <a href="{{ route('download_seller_products.products', ['id' => $seller->id]) }}" class="dropdown-item" download>
                                            {{ translate('Products List') }}
                                        </a>
                                    @endcan
                                    @can('download_seller_products')
                                        <a href="{{ route('download_seller_products.stocks', ['id' => $seller->id]) }}" class="dropdown-item" download>
                                    ...
                                    @can('seller_payment_history')
                                        <a href="{{ route('sellers.payment_history', encrypt($seller->user_id)) }}" class="dropdown-item">
