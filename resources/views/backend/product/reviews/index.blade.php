...
@section('content')
  <div class="aiz-titlebar text-left mt-2 mb-3">
    <div class="row align-items-center">
      ...
      <div class="col-md-6 text-md-right">
        @can('add_product_review')
          <a href="{{ route('reviews.demo-download') }}" class="btn btn-info" download>
            <span>{{ translate('Download Bulk Upload Excel Demo') }}</span>
          </a>
          <a href="#" class="btn btn-primary" data-target="#upload-reviews-modal" data-toggle="modal">
            <span>{{ translate('Bulk Add Reviews') }}</span>
          </a>
        @endcan
      </div>
    </div>
  ...
  <div class="card">
    <div class="card-header row gutters-5">
      <div class="col justify-center text-center text-md-left">
        <h5 class="mb-md-0 h6">{{ translate('Product Reviews') }}</h5>
      </div>
        ...
        <tbody>
          @foreach ($reviews as $key => $review)
            ...
            <tr>
              ...
              <td>{{ ucwords($review->product->added_by) }}</td>
              <td>{{ $review->user ? $review->user->name . ' (' . $review->user->email . ')' : 'Anonymous User' }}</td>
              <td>{{ $review->rating }}</td>
              <td>{{ $review->comment }}</td>
              <td>
                <label class="aiz-switch aiz-switch-success mb-0">
                  <input @can('publish_product_review') onchange="update_published(this)" @endcan
                    value="{{ $review->id }}" type="checkbox" @if ($review->status == 1) checked @endif
