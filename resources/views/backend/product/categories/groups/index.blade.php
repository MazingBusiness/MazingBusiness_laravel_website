    ...
    <div class="row align-items-center">
      ...
      <div class="col-md-6 text-md-right">
        @if (auth()->user()->can('add_product_category_groups'))
          <a href="{{ route('category-groups.create') }}" class="btn btn-info">
              ...
              <td class="text-right">
                @can('edit_product_category_groups')
                  <a class="btn btn-soft-primary btn-icon btn-circle btn-sm"
                    href="{{ route('category-groups.edit', ['id' => $category->id, 'lang' => env('DEFAULT_LANGUAGE')]) }}"
                ...
                @can('delete_product_category_groups')
                  <a href="#" class="btn btn-soft-danger btn-icon btn-circle btn-sm confirm-delete"
                    data-href="{{ route('category-groups.destroy', $category->id) }}" title="{{ translate('Delete') }}">
...
@section('script')
  <script type="text/javascript">
    function update_featured(el) {
      ...
      $.post('{{ route('category-groups.featured') }}', {
        _token: '{{ csrf_token() }}',
        id: el.value,
        status: status
      }, function(data) {
        if (data == 1) {
          AIZ.plugins.notify('success', '{{ translate('Featured Category Group updated successfully') }}');
        } else {
