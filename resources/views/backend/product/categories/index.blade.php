  ...
  <div class="aiz-titlebar text-left mt-2 mb-3">
    <div class="row align-items-center">
      ...
      <div class="col-md-6 text-md-right">
        ...
        @endcan
        <a href="{{ route('category-groups.index') }}" class="btn btn-primary">
          <span>{{ translate('View Category Groups') }}</span>
        </a>
      </div>
    </div>
  </div>
      ...
      <table class="table aiz-table mb-0">
        <thead>
          <tr>
            ...
            <th data-breakpoints="lg">{{ translate('Category Group') }}</th>
            ...
            <th data-breakpoints="lg">{{ translate('Markup') }}</th>
        ...
        <tbody>
          @foreach ($categories as $key => $category)
            <tr>
              ...
              <td>
                @if ($category->category_group_id != null)
              ...
              <td>
                @php
                    $file_path = $category->banner_image 
                  ? url('public/' . $category->icon_image) 
              ...
              <td>
                @if ($category->banner != null)
                  ...
                  <img src="{{ $file_path }}" alt="{{ translate('Banner') }}" class="h-50px">
                @else
                  —
                @endif
              </td>
              <td>
                @if ($category->icon != null)
                  <span class="avatar avatar-square avatar-xs">
                    ...
                    <img src="{{ $file_path }}" alt="{{ translate('icon') }}">
                  </span>
                @else
                  —
                @endif
              ...
              <td class="text-right">
                ...
                @can('delete_product_category')
                  <a href="#" class="btn btn-soft-danger btn-icon btn-circle btn-sm confirm-delete"
                    data-href="{{ route('categories.destroy', $category->id) }}" title="{{ translate('Delete') }}">
