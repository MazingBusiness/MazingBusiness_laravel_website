...
@endforeach --}}
              ...
              <div class="form-group row" id="seller">
                <label class="col-lg-3 col-from-label">{{ translate('Seller Name') }} <i class="las la-star text-danger" title="{{ translate('Required') }}"></i></label>
                <div class="col-lg-8">
                  <select class="form-control aiz-selectpicker" name="seller_id" id="seller_id"
                    data-selected="{{ $product->seller_id }}" data-live-search="true" required>
                    @foreach ($sellers as $seller)
                      <option value="{{ $seller->id }}">{{ $seller->user_name }}</option>
                    @endforeach
                  </select>
                </div>
              </div>
              ...
              @if(count($photos) > 0)
                ...
                @foreach($photos as $photo)
                  ...
                  <img src="{{ env('UPLOADS_BASE_URL') . '/' . $photo->file_name }}" class="img-rounded" alt="Cinque Terre" style="border: 1px solid #000000; border-radius: 10px; width:20%;">
                    ...
                    <!-- Delete Button -->
                <a style="width:150px;" href="javascript:void(0)" class="btn-danger dropdown-item confirm-alert"
                     data-href="{{ route('delete-file', ['part_no' => $photo->file_original_name]) }}" 
          ...
          <div class="card">
            <div class="card-header">
              <h5 class="mb-0 h6">{{ translate('Import Product Images') }}</h5>
            </div>
            <div class="card-body">
              ...
              <div class="form-group row">
                <label class="col-md-3 col-form-label">
                  {{ translate('Import Gallery Images') }}
                </label>
                <div class="col-md-8">
                  <div class="input-group" data-toggle="aizuploader" data-type="image" data-multiple="true">
                    <div class="input-group-prepend">
                    ...
                    {{-- Store selected upload IDs as CSV (same as main photos) --}}
                    <input type="hidden"
                           name="import_photos"
                           value="{{ $product->import_photos }}"
                           class="selected-files">
                  </div>
              ...
              <div class="form-group row">
                <label class="col-md-3 col-form-label">
                  {{ translate('Import Thumbnail Image') }}
                ...
                <div class="col-md-8">
                  <div class="input-group" data-toggle="aizuploader" data-type="image">
                    ...
                    <input type="hidden"
                           name="import_thumbnail_img"
                           value="{{ $product->import_thumbnail_img }}"
                           class="selected-files">
                  </div>
                  <div class="file-preview box sm"></div>
                </div>
            ...
            <div class="col-lg-1">
                <label class="aiz-switch aiz-switch-success mb-0">
                    <input value="1" type="checkbox" name="colors_active"
                        @if (count(json_decode($product->colors)) > 0) checked @endif>
                    <span></span>
                </label>
            </div>
        </div>
...
--}}
          ...
          <div class="card">
            ...
            <div class="card-body">
              <div class="form-group row">
                <label class="col-lg-3 col-from-label">{{ translate('Purchase Price') }} <i class="las la-star text-danger" title="{{ translate('Required') }}"></i></label>
                <div class="col-lg-6">
              ...
              <div class="form-group row">
                <label class="col-lg-3 col-from-label">{{ translate('MRP (Manager-41)') }}</label>
                <div class="col-lg-6">
                  <input
                    type="number"
                    step="0.01"
                    placeholder="{{ translate('MRP (Manager-41)') }}"
                    name="mrp_41_price"
                    class="form-control"
                    value="{{ old('mrp_41_price', $product->mrp_41_price) }}"
                  >
                  <small class="text-muted d-block mt-1">
                    {{ translate('If empty, Manager-41 will fall back to MRP.') }}
                  </small>
                </div>
              </div>
...
--}}
    ...
    <!-- attribute variation part start -->
      <div class="card">
          <div class="card-header">
              <h5 class="mb-0 h6">{{ translate('Edit Variations') }}</h5>
              <button type="button" class="btn btn-primary btn-sm float-right" data-toggle="modal" data-target="#addVariationModal">
                  {{ translate('Add Variation') }}
              </button>
          </div>
          <div class="card-body">
              <!-- Field for editing variant_parent_part_no -->
              <div class="form-group">
                  <label for="variant_parent_part_no">{{ translate('Variant Parent Part Number') }}</label>
                  <input 
                      type="text" 
                      id="variant_parent_part_no" 
                      class="form-control" 
                      name="variant_parent_part_no" 
                      value="{{ $product->variation_parent_part_no }}" 
                      placeholder="{{ translate('Enter parent part number') }}">
              </div>

              <!-- Fields for editing attribute variations -->
              @if (!empty($attributeVariations))
                  @foreach ($attributeVariations as $attribute)
                      <label for="name" class="d-flex align-items-center">
                          {{ $attribute['attribute_name'] }}

                          <!-- Checkbox for is_variation -->
                          <div class="ml-3">
                              <label class="aiz-switch aiz-switch-success mb-0">
                                  <input 
                                      type="checkbox" 
                                      name="is_variation[{{ $attribute['attribute_id'] }}]" 
                                      value="1" 
                                      @if ($attribute['is_variation'] == 1) checked @endif>
                                  <span></span>
                              </label>
                          </div>
                      </label>
                      ...
                      <div class="form-row">
                          <div class="form-group col-md-12">
                              <div class="editable-box-container">
                  ...
                  @endforeach
              @else
                  <p>{{ translate('No variations available for this product.') }}</p>
              @endif
          </div>
      </div>

    <!-- attribute variation part end -->
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <div class="modal-header bg-primary text-white">
                <h5 class="modal-title" id="addVariationModalLabel">{{ translate('Add New Variation') }}</h5>
            ...
            <div class="modal-body">
                <form id="addVariationForm" method="POST" action="{{ route('admin.add.variation') }}">
                    @csrf
                    <input type="hidden" name="product_id" value="{{ $product->id }}">

                    <!-- Fields for attributes and values -->
                    <div class="form-group">
                        <label for="attributeSelect">{{ translate('Select Attribute') }}</label>
                        <select class="form-control aiz-selectpicker" id="attributeSelect" name="selected_attribute" data-live-search="true">
                            <option value="">{{ translate('Select an existing attribute') }}</option>
                    ...
                    <div class="form-group">
                        <label for="newAttribute">{{ translate('Or Create New Attribute') }}</label>
                        <input type="text" id="newAttribute" class="form-control" name="new_attribute" placeholder="{{ translate('Enter new attribute name') }}">
                    </div>

                    <!-- Toggle for is_variation -->
                    <div class="form-group">
                        <label class="aiz-switch aiz-switch-success mb-0">
                            <input type="checkbox" name="is_variations" value="1" checked>
                            <span></span>
                        </label>
                        <span class="ml-2">{{ translate('Mark as Variation Attribute') }}</span>
                    </div>

                    <div class="form-group">
                        <label for="valueSelect">{{ translate('Select Value') }}</label>
                        <select class="form-control aiz-selectpicker" id="valueSelect" name="selected_value" data-live-search="true">
                            <option value="">{{ translate('Select an existing value') }}</option>
                    ...
                    <div class="form-group">
                        <label for="newValue">{{ translate('Or Create New Value') }}</label>
                        <input type="text" id="newValue" class="form-control" name="new_value" placeholder="{{ translate('Enter new value') }}">
                    </div>

                    <div class="modal-footer">
                        <button type="button" class="btn btn-secondary" data-dismiss="modal">{{ translate('Close') }}</button>
                        <button type="submit" class="btn btn-primary">{{ translate('Add Variation') }}</button>
                    </div>
                </form>
            </div>
        </div>
...
@section('script')
 ...
 $(document).on("click", ".confirm-alert", function () {
      ...
      $("#delete-modal .comfirm-link").attr("href", deleteUrl);
  });
  ...
  $('#group_id').on('change', function() {
        var groupId = $(this).val();  // Get selected group_id

        if (groupId) {
            $.ajax({
                url: '{{ route("get.categories.by.group") }}', // Laravel route
                type: 'GET',
                data: { group_id: groupId },
                beforeSend: function() {
                    $('#category_id').html('<option>Loading...</option>'); // Show loading state
                },
                success: function(response) {
                    ...
                    $('#category_id').selectpicker('refresh'); // Refresh the select dropdown
                },
                error: function(xhr, status, error) {
                    console.error("AJAX Error: ", status, error); // Log error to console
                    alert("Error fetching categories: " + xhr.responseText); // Show alert to user
                    $('#category_id').empty().append('<option value="">Error loading categories</option>');
                }
            });
        } else {
