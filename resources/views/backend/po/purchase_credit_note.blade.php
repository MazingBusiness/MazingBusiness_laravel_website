...
<div class="aiz-titlebar text-left mt-2 mb-3">
    <h1 class="h3 text-dark">💳 Credit Note</h1>
</div>
...
<div class="card">
    ...
    <div class="card-body">
           ...
           <form method="GET" action="{{ route('purchase.credit.note.list') }}" class="mb-4">
                <div class="input-group" style="max-width: 450px;">
                    ...
                    <div class="input-group-append">
                         @if(request('search'))
                            <a href="{{ route('purchase.credit.note.list') }}" class="btn btn-outline-secondary" title="Clear Search">
                                &times;
                            </a>
                        @endif
        ...
        <table class="table table-modern">
            ...
            <tbody>
                @foreach($purchases as $index => $purchase)
                    <tr @if (empty($purchase->zoho_creditnote_id)) style="background-color: #fff8dc;" @endif>
                        <td>{{ $index + 1 }}</td>
                        <td><strong>{{ $purchase->credit_note_number ?? '-' }}</strong></td>
                      ...
                      <td class="text-center" style="white-space: nowrap;">
                        <a href="{{ route('admin.export.credit_note_invoice', $purchase->id) }}" class="text-success mr-3" title="Download Excel" style="font-size: 15px; display: inline-flex; align-items: center;">
                            <i class="las la-file-excel" style="font-size: 22px; margin-right: 4px;"></i> Excel
                        </a>
                        <a href="{{ route('admin.credit_note.download_pdf', $purchase->id) }}" class="text-danger" title="Download PDF" style="font-size: 15px; display: inline-flex; align-items: center;">
