...
<div>
  <div class="aiz-titlebar text-left mt-2 mb-3">
    <div class="align-items-center">
      <h1 class="h3">{{ translate('Purchased Bag') }}</h1>
      @if (session('status'))
  ...
  <div class="card">
    ...
    <div class="card-body">
      <form id="purchase_order_form" method="GET" action="{{ route('admin.makePurchaseOrder') }}">
        @csrf
        <table class="table aiz-table mb-0">
          ...
          @foreach($purchaseOrders as $key => $order)
            <tr>
              ...
              <td>{{ $order->closing_qty < 0 ? 0 : $order->closing_qty }}</td>
              <td>{{ $order->to_be_ordered }}</td>
              <td>{{ $order->age }}</td>
              <td>
                <a href="{{ route('admin.deletePurchaseBagItem', ['id' => $order->id]) }}" 
