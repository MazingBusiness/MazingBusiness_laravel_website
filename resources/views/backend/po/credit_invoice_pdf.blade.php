...
<div class="page-border">
    ...
    <table class="top-info-section">
        <tr>
            <td width="50%">
                <strong>Purchase No.</strong>: {{ $invoice->credit_note_number }}<br>
                ...
                <strong>Purchase Order No.:</strong> {{ $invoice->purchase_order_no }}
            </td>
            <td width="50%">
                <span class="highlight">From:</span><br>
                {{ $customerInfo->company_name ?? '' }}<br>
                {{ $customerInfo->address ?? '' }} {{ $customerInfo->address_2 ? ', '.$customerInfo->address_2 : '' }}<br>
                @php
                    $city  = $customerInfo->city->name ?? $customerInfo->city ?? '';
                    $state = $customerInfo->state->name ?? $customerInfo->state_id ?? '';
                @endphp
                {{ trim(($city ? $city.', ' : '').($state ?: '')) }} {{ $customerInfo->postal_code ? '- '.$customerInfo->postal_code : '' }}<br>
