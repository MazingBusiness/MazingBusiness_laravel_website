...
<div class="aiz-titlebar text-left mt-2 mb-3 d-flex justify-content-between ">
    <h1 class="h3 text-dark">🏦 Debit Note</h1>
     <a href="{{ route('admin.viewDebitNotePurchaseOrder') }}" class="btn btn-primary">
        <i class="las la-plus-circle"></i> Add Debit Note
    </a>
</div>
...
<div class="card">
    ...
    <div class="card-body">
           ...
           <form method="GET" action="{{ route('purchase.debit.note.list') }}" class="mb-4">
                <div class="input-group" style="max-width: 450px;">
                    <input type="text" name="search" class="form-control" placeholder="Search by CN No, Supplier Info, Invoice No" value="{{ request('search') }}">
                    <div class="input-group-append">
                         @if(request('search'))
                            <a href="{{ route('purchase.debit.note.list') }}" class="btn btn-outline-secondary" title="Clear Search">
                                &times;
                            </a>
                        @endif
        ...
        <table class="table table-modern">
            ...
            <tbody>
                @foreach($purchases as $index => $purchase)
                    <tr @if (empty($purchase->zoho_debitnote_id)) style="background-color: #fff8dc;" @endif>
                        <td>{{ $index + 1 }}</td>
                        <td><strong>{{ $purchase->debit_note_number ?? '-' }}</strong></td>
                        <td>{!! str_replace(',', '<br>', $purchase->debit_note_no) !!}</td>
                        <td class="text-left">
                            @if ($purchase->address)
                                @php
                                ...
                                @if ($company || $phone || $fullAddress)
                                    <strong>
                                        {{ $company ?: 'No data found' }} (Customer)
                                        @if(!empty($purchase->debit_note_order_no))
                                            <span style="color: #888; font-weight: normal;"> - {{ $purchase->debit_note_order_no }}</span>
                                        @endif
                                    </strong><br>
                            ...
                            @php
                                $sellerInfo = is_array($purchase->seller_info)
                                    ? $purchase->seller_info
                                    : json_decode($purchase->seller_info, true);
                            @endphp
                            <strong>
                                {{ $sellerInfo['seller_name'] ?? 'No data found' }} (Seller)
                                @if(!empty($purchase->debit_note_order_no))
                                    <span style="color: #888; font-weight: normal;"> - {{ $purchase->debit_note_order_no }}</span>
                                @endif
                            </strong><br>
                            <small><i class="las la-phone"></i> {{ $sellerInfo['seller_phone'] ?? 'No data found' }}</small><br>
                            <small><i class="las la-map-marker"></i> {{ $sellerInfo['seller_address'] ?? 'No data found' }}</small>
                      ...
                      <td class="text-center" style="white-space: nowrap;">
                        <a href="{{ route('debit.note.export', $purchase->id) }}" class="text-success mr-3" title="Download Excel" style="font-size: 15px; display: inline-flex; align-items: center;">
                            <i class="las la-file-excel" style="font-size: 22px; margin-right: 4px;"></i> Excel
                        </a>
                        <a href="{{ route('debit.invoice.pdf', $purchase->id) }}" class="text-danger" title="Download PDF" style="font-size: 15px; display: inline-flex; align-items: center;">
                            <i class="las la-file-pdf" style="font-size: 22px; margin-right: 4px;"></i> PDF
                        </a>
                       @if ($purchase->purchase_invoice_type === 'customer' && $purchase->zoho_debitnote_id)
                            ...
                            @if ($purchase->credit_note_irp_status == 1)
                                ...
                                <button style="border: none;" class="text-danger cancel-irp-btn"
                                    data-id="{{ $purchase->zoho_debitnote_id }}"
                        ...
                        <td colspan="8">
                            <div class="p-3">
                                ...
                                <table class="table inner-table table-bordered mb-0">
                                    ...
                                    <tbody>
                                        @php $total = 0; @endphp
                                        @foreach($purchase->debitNoteInvoiceDetails as $i => $d)
                                            @php
