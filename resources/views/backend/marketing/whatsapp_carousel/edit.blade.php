  ...
  <div>
    ...
    @if (session('wa_create_error'))
      ...
      <div class="alert alert-danger" role="alert">
        <div class="font-weight-600 mb-1">Template save failed</div>
    ...
    <div class="meta-card">
      <div class="card-header d-flex align-items-center justify-content-between">
        <div>
          <div class="h6">Edit WhatsApp Media-Card Carousel</div>
          <div class="subtle">We’ll delete the old template, then create a new one.</div>
        </div>
        <a href="{{ route('wa.carousel.templates') }}" class="btn btn-ghost">Back to list</a>
      </div>

      <div class="card-body">
        <form action="{{ route('wa.carousel.edit-template.update') }}" method="POST" id="tplForm" novalidate>
          @csrf

          {{-- hidden old identity (from $prefill) --}}
          <input type="hidden" name="old_name" value="{{ data_get($prefill, 'name', '') }}">
          <input type="hidden" name="old_language" value="{{ strtolower(substr((string) data_get($prefill, 'language', 'en'),0,2)) }}">
          <input type="hidden" name="template_id" value="{{ data_get($prefill, 'id', '') }}">
          ...
          <div class="group">
            ...
            <div>
              <input type="text" name="name" class="form-control"
                     value="{{ old('name', data_get($prefill, 'name', '')) }}"
                     placeholder="media_carousel_v{{ date('Ymd_His') }}">
              <div class="subtle mt-1">Lowercase, numbers & underscores only. (If same as old, backend will auto-suffix.)</div>
            </div>
          </div>
          ...
          <div class="group">
            ...
            <div class="d-grid" style="grid-template-columns:1fr 1fr; gap:12px;">
              @php($lang = strtolower(substr((string) data_get($prefill,'language','en'),0,2)))
              <select name="language" class="form-control">
                <option value="en" {{ old('language',$lang)==='en' ? 'selected' : '' }}>English (en)</option>
                <option value="hi" {{ old('language',$lang)==='hi' ? 'selected' : '' }}>Hindi (hi)</option>
              </select>

              @php($hdr = strtoupper((string) data_get($prefill,'header_format','IMAGE')))
              <select name="header_format" class="form-control" id="headerFormat">
                <option value="IMAGE" {{ old('header_format',$hdr)==='IMAGE' ? 'selected' : '' }}>Header: IMAGE</option>
                <option value="VIDEO" {{ old('header_format',$hdr)==='VIDEO' ? 'selected' : '' }}>Header: VIDEO</option>
              </select>
            </div>
          </div>
          ...
          <div class="group">
            ...
            <div>
              <textarea name="body" id="topBody" rows="3" class="form-control">{{ old('body', data_get($prefill,'body','')) }}</textarea>
          ...
          <div class="group">
            ...
            <div id="top-body-examples">
              @php($examples = old('body_example', (array) data_get($prefill,'body_example',[])))
              @if(empty($examples)) @php($examples = ['Example 1','Example 2']) @endif
              @foreach($examples as $i => $ex)
                <div class="input-group mb-2 example-row">
                  <div class="input-group-prepend"><span class="input-group-text">Example {{ '{'.($i+1).'}' }}</span></div>
                  <input type="text" name="body_example[]" class="form-control" value="{{ $ex }}">
                </div>
              @endforeach
    ...
    <div class="meta-card">
      ...
      <div class="card-body">
        <div class="phone">
          ...
          <div class="subtle">Note: Original media URLs cannot be fetched from Meta. Paste new header links.</div>
        </div>
      </div>
    </div>
  ...
  <div class="meta-card card-item" data-index="__INDEX__">
    ...
    <div class="card-body">
      <div class="group">
        ...
        <div>
          <input type="url" class="form-control" name="cards[__INDEX__][header_link]" placeholder="https://example.com/image-or-video.jpg">
          <div class="subtle mt-1">Public URL. We’ll re-upload to a 4:: handle on save.</div>
        </div>
      </div>
      ...
      <div class="group">
        ...
        <div>
          ...
          <div class="subtle mt-1">Example must be a full URL. The template uses <code>https://.../product/@{{1}}</code>.</div>
        </div>
      </div>
...
{{-- Pass prefill + fallback to JS safely --}}
<script>
  window.__PREFILL__  = @json($prefill ?? []);
  window.__FALLBACK__ = @json('https://mazingbusiness.com/public/uploads/all/fRDJWDaZZqUjETWXHtdejKAmf2voEAXvUIUUeA8u.jpg');
</script>

@verbatim
<script>
(function(){
  const FALLBACK = window.__FALLBACK__ || '';
  ...
  function addCard(prefill={}){
    ...
    // Header link → default to FALLBACK if blank
    const preHeader = ((prefill.header_link || '') + '').trim();
    set(`[name="cards[${idx}][header_link]"]`, preHeader || FALLBACK);

    // Body + examples
    set(`[name="cards[${idx}][body_text]"]`, prefill.body_text || '');
    const exWrap = el.querySelector('[data-card-examples]');
    if (exWrap && Array.isArray(prefill.body_example)) {
      const inputs = exWrap.querySelectorAll('input[name="cards['+idx+'][body_example][]"]');
      inputs.forEach((inp,i)=>{ if(prefill.body_example[i]!=null) inp.value=prefill.body_example[i]; });
    }
  ...
  function renderPreview(){
    ...
    cards.forEach((card)=>{
      const rawLink = card.querySelector('input[name*="[header_link]"]')?.value || '';
      const link = (rawLink.trim() || FALLBACK);
  ...
  function qualityScan(){
    ...
    qualityPanel.innerHTML = issues.length
      ...
      : `<div class="success">Looks good. You’re ready to save.</div>`;
  }

  function seedFromPrefill(){
    const P = window.__PREFILL__ || {};
    const cards = Array.isArray(P.cards) ? P.cards : [];
    if (cards.length === 0) {
      // one empty card for convenience (header defaults to FALLBACK)
      addCard({
        header_link: '',
        body_text: 'Product highlight: {{1}} — reliable performance. Price {{2}} including GST.',
        body_example: ['Precision finishing', '₹1499'],
        url_btn_example: 'https://mazingbusiness.com/product/sample-slug',
    ...
    cards.forEach((c)=>{
      addCard({
        header_link: (c.header_link || ''), // defaulting handled inside addCard()
        body_text: (c.body_text || ''),
        body_example: Array.isArray(c.body_example) ? c.body_example : [],
        url_btn_example: (c.url_btn_example || ''),
        quick_reply_text: (c.quick_reply_text || 'Interested')
      });
    });
  }

  // Ensure blank header_link becomes FALLBACK on submit
  if (form){
    form.addEventListener('submit', ()=>{
      document.querySelectorAll('input[name^="cards"][name$="[header_link]"]').forEach(inp=>{
        if (!inp.value || !inp.value.trim()) inp.value = FALLBACK;
      });
    });
  }
