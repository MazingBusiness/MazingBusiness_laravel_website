            ...
            <table>
              <tr>
                <td colspan="2" style="padding-bottom: 15px;text-align: center;">
                  <span style="font-weight: bold">{{ translate('Quotations No.') }}:</span>
                  {{ $quotation_id }}
                </td>
              </tr>
              <tr>
                <td>{{ translate('Quotation Date') }}</td>
                <td class="text-right">{{ date('d/m/Y', strtotime($client_address->created_at)) }}</td>
              </tr>
              <tr style="display:none;">
                <td>{{ translate('Warehouse') }}</td>
                <td class="text-right">{{ $client_address->warehouse_name }}</td>
              </tr>
              <tr style="display:none;">
                <td>{{ translate('Credit Days') }}</td>
                <td class="text-right">{{ $client_address->credit_days }}</td>
              </tr>
              <tr style="display:none;">
                <td>{{ translate('Credit Limit') }}</td>
                <td class="text-right">{{ $client_address->credit_limit }}</td>
              </tr>
            </table>
          </td>
        </tr>
          ...
          @foreach ($cart_items as $key => $item)
              <tr>
                ...
                @php
                  ...
                  // Construct thumbnail path based on whether the thumbnail exists
                  $thumbnailPath = $item->thumbnail 
                      ? $baseUrl . '/' . $item->thumbnail 
                      : asset('assets/img/placeholder.jpg');
                @endphp

                  <img style="width:50px;height:50px;" class="img-fit lazyload mx-auto" src="{{ $thumbnailPath }}" alt="" onerror="this.onerror=null;this.src='{{ static_asset('assets/img/placeholder.jpg') }}';">
    ...
    <div style="padding: 0 0.8rem;">
      <table class="bank-details-section" style="width: 100%; margin-top: 5px;">
        <tr>
          ...
          <td style="width: 50%; text-align: right; padding: 10px;"> <!-- Reduced padding  https://admin.mazingbusiness.com/assets/pdf/mazing_business_qr.jpg -->
